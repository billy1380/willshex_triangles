...
void main() {
  ...
  stdout.writeln(
      "Color: ${ColorUtils.red(color)}, ${ColorUtils.green(color)}, ${ColorUtils.blue(color)}");
  ...
  for (int i = 0; i < quantizer.quantizedColors.length; i++) {
    ...
    stdout.writeln(
        "  Color $i: R=${(color.red * 255).round()}, G=${(color.green * 255).round()}, B=${(color.blue * 255).round()}");
  }
  ...
  stdout.writeln(
      'Vibrant color: ${palette.vibrantColor != null ? "Found" : "Not found"}');
  stdout.writeln(
      'Muted color: ${palette.mutedColor != null ? "Found" : "Not found"}');
  stdout.writeln(
      'Dark vibrant color: ${palette.darkVibrantColor != null ? "Found" : "Not found"}');
  stdout.writeln(
      'Light vibrant color: ${palette.lightVibrantColor != null ? "Found" : "Not found"}');
  stdout.writeln(
      'Dark muted color: ${palette.darkMutedColor != null ? "Found" : "Not found"}');
  stdout.writeln(
      'Light muted color: ${palette.lightMutedColor != null ? "Found" : "Not found"}');
  ...
  stdout.writeln(
      "RGB(100, 150, 200) -> HSL(${hsl[0].toStringAsFixed(1)}, ${hsl[1].toStringAsFixed(3)}, ${hsl[2].toStringAsFixed(3)})");

  final int convertedColor = ColorUtils.hslToRgb(hsl);
  stdout.writeln(
      "HSL -> RGB: ${ColorUtils.red(convertedColor)}, ${ColorUtils.green(convertedColor)}, ${ColorUtils.blue(convertedColor)}");
  ...
  stdout.writeln(
      "Contrast between white and black: ${contrast.toStringAsFixed(3)}");
}
...
class ImageColorAnalyzer {
  ...
  static CanvasSamplePalette analyzeImage(List<int> imagePixels,
      {int maxColors = 16}) {
    return CanvasSamplePalette.generateWithColors(imagePixels, maxColors);
  }

  /// Get the most prominent colors from an image
  static List<Color> getProminentColors(List<int> imagePixels,
      {int count = 5}) {
    final CanvasSamplePalette palette =
        CanvasSamplePalette.generate(imagePixels);
    final List<Color> prominentColors = <Color>[];

    // Add vibrant colors first
    if (palette.vibrantColor != null) {
      prominentColors.add(palette.vibrantColor!);
    }
    if (palette.lightVibrantColor != null) {
      prominentColors.add(palette.lightVibrantColor!);
    }
    if (palette.darkVibrantColor != null) {
      prominentColors.add(palette.darkVibrantColor!);
    }

    // Add muted colors
    if (palette.mutedColor != null) prominentColors.add(palette.mutedColor!);
    if (palette.lightMutedColor != null) {
      prominentColors.add(palette.lightMutedColor!);
    }
    if (palette.darkMutedColor != null) {
      prominentColors.add(palette.darkMutedColor!);
    }
